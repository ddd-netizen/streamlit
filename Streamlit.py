...
st.markdown(
    '<style>div.block-container{padding-top:1rem;}<s/style>',
...
if fl is not None:
    ...
    os.chdir(r"C:\Users\LOLT\OneDrive\Desktop\Streamlit")
...
with col2:
    ...
    if not state:
        ...
        df3=df[df2["State"].isin(state)]

#Create for city
city=st.sidebar.multiselect("Pick Your City",df3["City"].unique())

#filtered date based on  Region,state and city
    ...
    with cl1:
        with st.expander("category_viewData"):
            ...
            st.download_button("Download data",data=csv,file_name="Category.csv",
                                mime="txt/csv",help = 'Click here to download the data as a CSV file')
with cl2:
...
with chart1:
    ...
    with chart2:
        ...
        with st.expander("Summary_Table"):
            df_sample=df[["Region","State","City","Category","Sales","Profit","Quantity"]]
            fig=ff.create_table(df_sample,colorscale="Cividis")
            st.plotly_chart(fig,use_container_width=True)

            st.markdown("Month wise Sales Category Table")
            ...
            with st.exception("View data"):
                st.write(filtered_df.iloc[:500,1:20:2].style.background_gradient(cmap="Orange"))
